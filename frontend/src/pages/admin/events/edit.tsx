  ...
  useEffect(() => {
    if (data?.data?.data) {
      ...
      setForm({
        ...
        min_choices: event.min_choices || 1,
        max_choices: event.max_choices || 1,
        allow_multiple_choices: event.allow_multiple_choices || false,
        is_result_public: event.is_result_public || false,
      })
    }
  }, [data])

  const updateMutation = useMutation({
    mutationFn: (d: typeof form) => eventsAPI.update(id!, d),
    onSuccess: () => {
  ...
  return (
    <div className="max-w-full space-y-6 animate-fade-in">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Edit Event Voting</h1>
          <p className="text-muted-foreground text-sm">Ubah rincian dan konfigurasi event pemilihan ini.</p>
        </div>
      </div>
