...
export default function CreateVoterPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (d: typeof form) => votersAPI.create(d),
    onSuccess: () => {
      toast.success('Voter berhasil ditambahkan!')
  ...
  return (
    <div className="max-w-full space-y-6 animate-fade-in">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Tambah Voter</h1>
          <p className="text-muted-foreground text-sm">Masukkan data pemilih baru ke dalam sistem.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="sticky bottom-0 z-50 flex justify-end gap-3 bg-background/95 backdrop-blur border-t p-4 -mx-4 md:-mx-6 lg:-mx-8 mt-8">
          ...
          <Button type="submit" disabled={createMutation.isPending} className="shadow-xs">
            {createMutation.isPending ? 'Menyimpan...' : 'Simpan Pemilih'}
          </Button>
        </div>
      </form>
    </div>
